...
static int msm_eeprom_platform_probe(struct platform_device *pdev)
{
	...
	if (e_ctrl->userspace_probe == 0) {
		...
		if (strcmp(eb_info->eeprom_name, "chiron_imx386_semco") == 0) {
			 ...
			 gyro_cal_data_res[0] = -0.015717;
			 gyro_cal_data_res[1] = -0.015656;
			 temp = (int16_t)((((gyro_cal_data_res[1] * 180) / 3.1415926) * 32768) / 250);
			 g_cal_fadj_data[19]  = (temp & 0xFF);
			 g_cal_fadj_data[20] = ((temp >> 8) & 0xFF);
			 temp = (int16_t)((((gyro_cal_data_res[0] * 180) / 3.1415926) * 32768) / 250);
			g_cal_fadj_data[21]  = (temp & 0xFF);
			g_cal_fadj_data[22] = ((temp >> 8) & 0xFF);
			pr_err(" g_cal_fadj_data %x %x %x %x",
			g_cal_fadj_data[19], g_cal_fadj_data[20], g_cal_fadj_data[21], g_cal_fadj_data[22]);
 		}
#endif
